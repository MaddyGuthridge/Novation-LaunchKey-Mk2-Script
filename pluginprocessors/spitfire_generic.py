...
def setExpression(command):
    ...
    EXPECTED_EXPRESSION = pluginswrapper.setParamByName("Expression", command.value, -1, EXPECTED_EXPRESSION)
    command.handle("Set expression", 1)

def setDynamics(command):
    global EXPECTED_DYNAMICS
    EXPECTED_DYNAMICS = pluginswrapper.setParamByName("Dynamics", command.value, -1, EXPECTED_DYNAMICS)
    command.handle("Set dynamics", 1)

def setReverb(command):
    global EXPECTED_REVERB
    EXPECTED_REVERB = pluginswrapper.setParamByName("Reverb", command.value, -1, EXPECTED_REVERB)
    command.handle("Set reverb", 1)

def setRelease(command):
    global EXPECTED_RELEASE
    EXPECTED_RELEASE = pluginswrapper.setParamByName("Release", command.value, -1, EXPECTED_RELEASE)

def setTightness(command):
    global EXPECTED_TIGHTNESS
    EXPECTED_TIGHTNESS = pluginswrapper.setParamByName("Tightness", command.value, -1, EXPECTED_TIGHTNESS)
